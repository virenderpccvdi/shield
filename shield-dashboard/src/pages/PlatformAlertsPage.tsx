import {
  ...
  Alert, Stack, CircularProgress, TextField, InputAdornment, Divider, Tooltip,
} from '@mui/material';
...
function SosPanelSection({ profileNameMap }: { profileNameMap: Record<string, string> }) {
  const qc = useQueryClient();
  const [sosTab, setSosTab] = useState<'active' | 'history'>('active');

  const { data: allSos = [], isLoading } = useQuery<SosEvent[]>({
    queryKey: ['platform-sos-all'],
    queryFn: () =>
      api.get('/location/sos/platform?all=true')
        .then(r => (r.data?.data ?? r.data ?? []) as SosEvent[])
        .catch(() => []),
    refetchInterval: 30000,
  });

  const activeSos  = allSos.filter(e => e.status === 'ACTIVE');
  const historySos = allSos.filter(e => e.status !== 'ACTIVE');

  const profileLabel = (id: string) => profileNameMap[id] || `Profile …${id.slice(-8)}`;

  const ackMutation = useMutation({
    mutationFn: (id: string) => api.post(`/location/sos/${id}/acknowledge`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['platform-sos-all'] }),
  });
    ...
    <Box sx={{ mb: 3 }}>
      ...
      {activeSos.length > 0 && (
        <Alert severity="error" icon={false} sx={{
          ...
          🚨 {activeSos.length === 1
            ? `1 active SOS emergency — ${profileLabel(activeSos[0].profileId)}`
            : `${activeSos.length} active SOS emergencies across the platform`
          }{' '}Respond immediately.
        </Alert>
      )}
          ...
          : <Stack spacing={1.5}>
            {historySos.map(sos => (
              <Box key={sos.id} sx={{
                ...
                <Box sx={{ flex: 1 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5, flexWrap: 'wrap' }}>
                    <Typography fontWeight={700} variant="body2">{profileLabel(sos.profileId)}</Typography>
...
export default function PlatformAlertsPage() {
  ...
  // Fetch all child profiles for name enrichment (platform-wide)
  const { data: allProfiles = [] } = useQuery<ChildProfile[]>({
    queryKey: ['platform-all-profiles'],
    queryFn: () =>
      api.get('/profiles/admin/children?size=200')
        .then(r => { const d = r.data?.data; return (d?.content ?? d ?? []) as ChildProfile[]; })
        .catch(() => []),
    staleTime: 300000,
  });
  const profileMap = useMemo(() =>
    Object.fromEntries(allProfiles.map(p => [p.id, p.name])), [allProfiles]);

  // Fetch notification history for current admin/ISP user
  const { data: historyAlerts = [], isLoading } = useQuery<AlertItem[]>({
    queryKey: ['platform-notification-history'],
    queryFn: async () => {
      const r = await api.get('/notifications/my?page=0&size=100');
  ...
  // Mark as read — backend + in-memory
  const markReadMutation = useMutation({
    mutationFn: (id: string) => api.put(`/notifications/${id}/read`),
    onSuccess: (_, id) => {
      useAlertStore.getState().markRead(id);
      qc.invalidateQueries({ queryKey: ['platform-notification-history'] });
    },
  });
  ...
  return (
    <AnimatedPage>
      <PageHeader
        ...
        subtitle={`${allAlerts.length} notifications · ${unreadCount} unread · ${allProfiles.length} child profiles monitored`}
        iconColor="#E53935"
        action={
      ...
      <SosPanelSection profileNameMap={profileMap} />
        ...
        <Card>
          ...
          <Box sx={{ px: 2, pt: 1.5, pb: 0, borderBottom: '1px solid #F1F5F9' }}>
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems={{ sm: 'center' }} sx={{ mb: 1 }}>
              <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{
                ...
                <Tab label={
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                    <PeopleIcon sx={{ fontSize: 16 }} /> All Notifications
                  </Box>
                } />
                <Tab label={
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
              ...
              <TextField size="small" placeholder="Search by message, profile, type..."
          ...
          {isLoading && filtered.length === 0 ? (
            ...
            <EmptyState
              ...
              description={tab === 1 ? 'No unread alerts' : search ? 'Try adjusting your search' : 'Platform alerts will appear here when triggered'}
            />
          ) : (
            <Box>
