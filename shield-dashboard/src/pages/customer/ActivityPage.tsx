...
export default function ActivityPage() {
  ...
  const filtered = events.filter(e =>
    ...
    (!search || e.domain.includes(search.toLowerCase()))
  );
  ...
  return (
    <AnimatedPage>
      <PageHeader
        icon={<TimelineIcon />}
        title="Live Activity"
        subtitle="Real-time DNS query monitoring"
        iconColor="#1565C0"
      />
                ...
                {filtered.slice(0, 100).map((ev, i) => (
                  ...
                  >
                    <Typography variant="caption" color="text.secondary" sx={{
                      minWidth: 80, fontFamily: '"JetBrains Mono", monospace', fontSize: 11,
                      bgcolor: '#F8FAFC', px: 1, py: 0.3, borderRadius: 1,
                    }}>
