...
export default function ChildProfilesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-child-profiles', search, page, size],
    queryFn: () => {
      const params = new URLSearchParams({ page: String(page), size: String(size) });
      if (search.trim()) params.set('search', search.trim());
      return api.get(`/profiles/admin/children?${params}`).then(r => r.data?.data || r.data)
        .catch(() => ({ content: [], totalElements: 0 }));
    },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) => api.put(`/profiles/admin/children/${id}`, body),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-child-profiles'] }); setSnack('Profile updated'); setEditOpen(false); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/profiles/admin/children/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-child-profiles'] }); setSnack('Profile deleted'); setDeleteOpen(false); },
  });
  ...
  return (
    <AnimatedPage>
      <PageHeader
        ...
        subtitle={`${total} profile${total !== 1 ? 's' : ''} across the platform`}
        iconColor="#7B1FA2"
        action={
              ...
              return (
                <Grid size={{ xs: 12, sm: 6, md: 4 }} key={profile.id}>
                  <AnimatedPage delay={i * 0.04}>
                    ...
                    }} onClick={() => navigate(`/admin/child-profiles/${profile.id}`)}>
                      <Box sx={{ height: 4, background: 'linear-gradient(135deg, #7B1FA2 0%, #4A148C 100%)' }} />
                      <CardContent>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                          <Avatar sx={{ width: 48, height: 48, background: 'linear-gradient(135deg, #7B1FA2 0%, #4A148C 100%)', fontWeight: 700, fontSize: 16 }}>
                        ...
                        <Button fullWidth size="small" variant="outlined" startIcon={<ShieldIcon />}
                          sx={{ mt: 2, borderRadius: 2, borderColor: '#7B1FA220', color: '#7B1FA2', '&:hover': { borderColor: '#7B1FA2', bgcolor: '#7B1FA208' } }}
                          onClick={(e) => { e.stopPropagation(); navigate(`/admin/child-profiles/${profile.id}`); }}>
                          Manage
                        </Button>
                      </CardContent>
                    </Card>
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ px: 3, py: 2 }}>
          ...
          <Button variant="contained" disabled={updateMutation.isPending}
            ...
            sx={{ bgcolor: '#7B1FA2', minWidth: 120 }}>
