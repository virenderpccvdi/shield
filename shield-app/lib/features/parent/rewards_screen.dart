...
class _RewardsScreenState extends ConsumerState<RewardsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      try {
        ...
        _bank = Map<String, dynamic>.from(bankRes.data['data'] as Map? ?? {});
      } catch (_) { _bank = {}; }
    } catch (_) {
      _tasks = [];
    }
  ...
  Future<void> _approveTask(String taskId) async {
    try {
      final client = ref.read(dioProvider);
      await client.post('/rewards/tasks/$taskId/approve');
      _load();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Task approved!'), backgroundColor: Colors.green),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _rejectTask(String taskId) async {
    try {
      final client = ref.read(dioProvider);
      await client.post('/rewards/tasks/$taskId/reject');
      _load();
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    final totalEarned = _bank['totalEarned'] as int? ?? 0;
